...
const ReportsPage = () => {
    ...
    // ── No dummy data — start with empty array, show spinner until fetch resolves ──
    const [reports, setReports] = useState([]);
    const [isLoading, setIsLoading] = useState(true); // stays true until fetchReportsAPI finishes
    ...
    // ─── Fetch ────────────────────────────────────────────────────────────────
    // setIsLoading(false) is now inside the fetch so the spinner shows until
    ...
    const fetchReportsAPI = async () => {
        ...
        try {
            ...
            setIsLoading(false); // always clear spinner when fetch settles
        }
    };
    ...
    const processedReports = useMemo(() => {
        ...
        if (filterDate) {
            filtered = filtered.filter((r) =>
                formatGenerationDate(r.generationDate).toLowerCase().includes(filterDate.toLowerCase())
            );
        }
        if (sortConfig.key) {
            filtered.sort((a, b) => {
                ...
                if (sortConfig.key === 'generationDate') { valA = new Date(valA); valB = new Date(valB); }
