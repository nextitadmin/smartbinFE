...
function ProfilePage() {
    ...
    const { updateCorporateInfo, fetchCorporateInfo } = useCorporateStore();
    ...
    const handleImageChange = async (e) => {
        ...
        try {
            // 1. Upload file to media storage to obtain hosted URL
            const uploadResult = await uploadFile(file);
            if (uploadResult && uploadResult.url) {
                const uploadedUrl = uploadResult.url;

                // 2. Patch corporate profile picture
                const { data } = await api.patch('/corporate/profile-picture', {
                ...
                if (data.success || data.succeeded) {
                    ...
                    updateCorporateInfo?.('passport', uploadedUrl);
                    fetchCorporateInfo?.();
                } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { data } = await api.put('/corporate/profile', dataToSend); // Or axios.post // Log successful response
